...
\newline
Die Funktion:
$$f(\text{cell}, \text{neighbour}) \rightarrow \text{direction}$$
Für das Gitter:
$$Gitter(\text{c}, \text{n}) =  
\begin{cases}  
    \text{Osten} & \text{wenn } x_n - x_c = 1 \text{ und } y_n - y_c = 0 \\
    \text{Norden} & \text{wenn } x_n - x_c = 0 \text{ und } y_n - y_c = 1 \\
    \text{Westen} & \text{wenn } x_n - x_c = -1 \text{ und } y_n - y_c = 0 \\
    \text{Süden} & \text{wenn } x_n - x_c = 0 \text{ und } y_n - y_c = -1
\end{cases}$$

////////////////////////////////////////////////

Für das normal quadratische Gitter:
$$\mathbf{d} \;=\; \text{neighbour} - \text{cell} = (x_n-x_c,\; y_n-y_c)$$
...
$$\mathbf{W} \;=\; Gitterbreite \text{und} \mathbf{H} \;=\; Gitterh"ohe$$

$$\mathbf{d} \;=\; \operatorname{wrap}\big(\text{neighbour} - \text{cell},\; (W,H)\big) \qquad\text{mit}\qquad \text{cell}=(x_c,y_c),\ \text{neighbour}=(x_n,y_n)$$

Komponentweise:
$$d_x = \operatorname{wrap}(x_n-x_c, W),\qquad d_y = \operatorname{wrap}(y_n-y_c, H)$$
...
\\
@todo(viktor): @incomplete Diagonale Regeln, besseres Sampling der Eingabe, Regelmengen , höhere Entropie durch Strictness