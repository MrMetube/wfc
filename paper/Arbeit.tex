...
% Abgabe:
% 
\@todo(viktor):{senden Sie ein PDF der Arbeit und der unterschriebenen Selbstständigkeitserklärung in einer E Mail an Ihren Betreuer, den Zweitgutachter, das Prüfungsamt (pruefungsamt-koethen@hs-anhalt.de) und Professor Schneider.}
...
\chapter{Ergebnisse und Diskussion}
    \at{@incomplete einleitender Satz, alles ausformulieren}

    \section{allgemein, aussehen, muster \at{@naming}}
        \begin{itemize} 
        ...
        \at{@placement Artefakte}
        Auch bei Graphen kann die Ausgabe visuelle Artefakte des Graphens enthalten. In Abbildung \ref{fig:cell_size} sind die Voronoi-Zellen erkennbar. Nun muss der Graph aber nicht so dargestellt werden. Den Knoten des Graphen wurden konsistente Zustände aus dem Beispiel zugewiesen. Danach könnten andere Methoden oder Postprocessing angewendet werden, um die Ausgabe zugestallten. In der Abbildung wird beispielhaft die Größe der Zellen verkleinert, wodurch die harten Kanten der Sechecke etwas weniger betont erscheinen, aber nun auch der Hintergrund zu sehen ist. Hier könnte man auf andere Weise die Lücken füllen, so dass die secheckige Form der Zellen kachiert wird. 
        
        \at{@placement im hintergrund schon erklären und dann hier höchsten nochmal erwähnen}
        \at{@visual des spiels} 
        Natürlich muss die Ausgabe auch nicht direkt bildlich verwendet werden. Welche Bedeutung die Knoten des Graphs und deren Zustände haben, ist dem Algorithmus egal. Dieser garantiert nur lokale Ähnlichkeit zum Beispiel. Ein anderes Beispiel für eine solche Weiterverarbeitung ist das erfolgreiche Spiel \textit{Townscaper} \cite{stalberg_townscaper} \at{@visual} von Oskar Stålberg. Hier kann die Spielerin aus Gebäuden eine Stadt bauen. Sie wählt dabei aber nicht die konkreten Gebäude sondern legt nur fest, welche Art/Farbe an jeweiliger Stelle platziert werden soll. Das Spiel nutzt Wave Function Collapse, um die dementsprechend am besten passenden Gebäudeteile zu finden. Damit die Stadt nicht geradlinig ist, also Artefakte des unterliegenden Gitters zeigt wird auch eine spezielle Art von Gitter verwendet. Es handelt sich dabei um eine Quadrilierung, also ein Graph basiert auf Vierecken. Diese werden \at{@visual ref auf gitter von Townscaper} \cite{stalberg_grid}
        
        \input{data/townscaper_grid/figure.tex}
        
        Abbildung \ref{fig:townscaper_grid} zeigt, wie die Teile, aus denen das Gitter für Townscaper besteht, generiert werden. Es ist zu erkennen, dass es auf einer Quadrilierung, also ein Gitter aus Vierecken, aufbaut. Die sechseckigen Teile werden dann im Spiel zusammengesetzt und an den Knoten kann die Spielerin Gebäude bauen. Die meisten Knotenpunkte haben vier Kanten, aber es gibt auch einzelne mit drei oder fünf Kanten. Im Spiel existieren für diese Arten von Knoten jeweils unterschiedliche Modelle. Es müssen keine weiteren Modelle für willkürlich viele Kanten an einem Knoten erstellt werden, da dieser Prozess nur diese drei Arten von Knoten generiert.
        
    
    
    
    \section{Der Effekt von Heat auf die Ausgabe}
        Im folgenden Abschnitt werden die positiven wie auch negativen Effekt von Heat mittels Beispielen erklärt. \at{@flow reihenfolge überdenken}
        
        % ////////////////////////////////////////////////
        
        \input{data/hex_heat/figure.tex}
        
        Abbildung \ref{fig:hex_heat} zeigt den Einfluss von Heat auf die Qualität der Ausgabe bei gleichem Beispiel und Graph. Das Beispiel kann auf dem Quadratgitter ohne weitere Probleme genutzt werden, siehe (b), während der Graph mit den Sechsecken bei geringer Heat uninteressante Ausgaben generiert, siehe (c).
        ...
        \at{@incomplete} \at{@visual weave beispiel mit vergleich der entropie nach einem Schritt mit höherere heat: effekt ist, dass die entropie entfernter zellen höher ist und nur direkte nachbar beeinfluss werden}
...
\chapter{Zusammenfassung und Ausblick}
    Dieses Kapitel bietet einen Rückblick auf die erfolgreiche Erweiterung des Wave Function Collapse Algorithmus. Auch werden noch bestehende Schwachstellen der präsentierten Methodik erklärt. Diese könnten als Ansatz für weiterführende Forschung und Entwicklung der Algorithmus dienen.
    ...
    \section{Zukünftige Forschung}
        \at{@incomplete einleitungs satz}
